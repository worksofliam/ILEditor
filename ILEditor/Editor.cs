...
namespace ILEditor
{
	public partial class Editor : Form
	{
		...
		public void AddTool(DockContent Content, DockState dock = DockState.Document, bool Replace = false)
		{
			...
			if (Replace)
				for (var x = 0; x < dockingPanel.Panes.Count; x++)
				{
					var currentPane = dockingPanel.Panes[x];
		...
		public static void ApplyControlTheme(Control content)
		{
			if (DarkMode)
			{
				if (content is ToolStrip strip)
					strip.Renderer = new ToolStripRenderer();

				content.BackColor = Color.FromArgb(45, 45, 48);
				content.ForeColor = Color.White;

				foreach (Control subControl in content.Controls)
					ApplyControlTheme(subControl);
			}
		}
		...
		public static void OpenSource(RemoteSource Source)
		{
			...
			var text = Source.GetName() +
			                 (Source.GetExtension() != "" ? '.' + Source.GetExtension().ToLower() : "");
			...
			new Thread((ThreadStart) delegate
			{
				switch (Source.GetFS())
				{
					case FileSystem.QSYS:
						resultFile = IBMiUtils.DownloadMember(Source.GetLibrary(),
							Source.GetObject(),
							Source.GetName(),
							Source.GetExtension());
				...
				if (resultFile != "")
				{
					...
					sourcePanel = new SourceEditor(Source.GetLocalFile(),
						GetBoundLangType(Source.GetExtension()),
						Source.GetRecordLength(),
						!Source.IsEditable());
		...
		{
			var text = Source.GetName() + (Source.GetExtension() != "" ? '.' + Source.GetExtension().ToLower() : "");

			if (File.Exists(Source.GetLocalFile()))
			{
				var sourcePanel = new SourceEditor(Source.GetLocalFile(),
					GetBoundLangType(Source.GetExtension()),
					Source.GetRecordLength(),
					!Source.IsEditable());
			...
			{
				MessageBox.Show("There was an error opening the local file. '" +
				                Source.GetLocalFile() +
				                "' does not exist");
			}
		}
		...
		{
			if (Tab is SourceEditor tabEditor)
				return tabEditor;
	...
	#region Compile Dropdown
		...
		private void compileToolStripMenuItem_DropDownOpening(object sender, EventArgs e)
		{
			...
			if (sourceInfo != null)
			{
				var items = IBMi.CurrentSystem.GetValue("TYPE_" + sourceInfo.GetExtension().ToUpper()).Split('|');
	...
	#region Source dropdown
		...
		private void cLFormattingToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			var sourceInfo = (RemoteSource) LastEditing.Tag;
			var language   = GetBoundLangType(sourceInfo.GetExtension());
