...
namespace ILEditor.Forms
{
	public partial class Connection : Form
	{
		private static readonly Dictionary<string, string> DataConnectionTypes = new Dictionary<string, string>
		{
			{
				"AutoPassive",
				"Default option. This type of data connection attempts to use the EPSV command and if the server does not support EPSV it falls back to the PASV command before giving up unless you are connected via IPv6 in which case the PASV command is not supported."
			},
			{
				"PASV",
			...
			{
				"PASVEX",
				"Same as PASV except the host supplied by the server is ignored and the data connection is made to the same address that the control connection is connected to. This is useful in scenarios where the server supplies a private/non-routable network address in the PASV response. It's functionally identical to EPSV except some servers may not implement the EPSV command. Please note that IPv6 does not support this type data connection. If you ask for PASV and are connected via IPv6 EPSV will automatically be used in its place."
			},
			{
				"EPSV",
				"Extended passive data connection, recommended. Works the same as a PASV connection except the server does not dictate an IP address to connect to, instead the passive connection goes to the same address used in the control connection. This type of data connection supports IPv4 and IPv6."
			},
			{
				"AutoActive",
				"This type of data connection attempts to use the EPRT command and if the server does not support EPRT it falls back to the PORT command before giving up unless you are connected via IPv6 in which case the PORT command is not supported."
			},
			{
				"PORT",
		...
		{
			infoBox.Text = DataConnectionTypes[dataConnectionType.SelectedItem.ToString()];
		}
	}
}